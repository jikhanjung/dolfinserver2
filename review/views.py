...
def reid(request):
    ...
    **모델이 판단하고 사람이 고치는 것이 아니라, 사람이 판단하고 모델은
    ...
    닮아 있어야 눈이 덜 움직인다. `rough` 는 notch 가 깊은 것부터. `day` 는 날짜순.
    ...
    if ready:
        ...
        # **개체가 아닌 자리 셋은 늘 있다** (`Individual.KINDS`).
        ...
        # `식별 불가능` — 지느러미는 맞는데 이 사진으로는 개체를 못 알아보는
        # 것. 화면에서는 고정칸이 아니라 **상자 목록 맨 끝**에 선다 — 판단이
        # 끝난 통이라 늘 닿아 있을 이유가 없다.
        ...
        **d,
        "bars": [
            ...
            ("조각", bar(d["n_chip"], g["chips"]),
             "개체를 찾으려면 격자를 훑어야 하고, 훑으면 조각이 따라 는다 — "
             "그 자체가 지렛대라서가 아니다"),
            (f"{g['per_individual']}장 이상인 개체", bar(d["at_chips"], d["n_ind"]),
             "쫓지 말 것 — 개체 수만 맞춘 대조군과 비겼다(59.9 대 60.7). "
             "보고만 있는 칸이다"),
