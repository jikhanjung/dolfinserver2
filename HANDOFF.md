...
얼마나 잘 찾느냐이고, 그것이 곧 사람이 하나하나 안 붙여도 되게 하는 길이다.
    ...
    **재는 날을 돌려서 잰다** (`reid_cls --folds 5 --seeds 3`) — 질의 493
    기준선 — 가장 닮은 조각의 개체(kNN)               27.4%     63.5%
    **분류기** (`reid_cls`)                        **45.0%**   75.1%
      씨앗 셋을 흔들어도 44.9 ± 0.1
...
**가중치 감쇠가 0~1e-2 에서 성적을 한 자리도 안 움직였다.** 과적합이 아니라
**자료를 다 못 담고 있다**는 신호라, 머리를 키울 여지가 남았다 — 2층 MLP ·
ArcFace · 백본 마지막 블록 녹이기(이것만 GPU 몫).
