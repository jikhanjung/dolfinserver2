...
변하고(결각이 새로 생긴다), 지금 덮개가 고르지 않다(62장짜리와 1장짜리).
...
정렬  닮은 것끼리(임베딩 사슬) · 특징적인 것부터(결각) · 날짜순
...
"특징적" 이었다. 진짜 결각은 2~5%다 — 볼록껍질과의 차이는 결각이 아니라
...
꼭짓점을 24로 줄였다: 마스크는 중간값 65개인데 결각을 보는 데 그만큼 필요
