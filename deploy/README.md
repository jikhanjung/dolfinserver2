...
`--derived` 로 크롭·조각까지 NAS 로). **2026-09-07 부터 날마다 04:40 에 돈다**
(`cd <저장소> && python manage.py backup`, 로그는 `logs/backup-nas.log`).
거는 까닭은 **녹은 블록**이다 — `reid/blocks/*.blocks.pt`(4.1GB)가 화면에 떠
있는 앙상블 멤버의 임베딩을 내는 것인데, 다시 배우면 씨앗과 정답 시점이 달라져
**다른 모델**이 된다. 가중치를 뜨는 까닭과 한 글자도 다르지 않다. 재차 회차는
**51초**다(내용이 같으면 건너뛴다).
**cron 은 `$HOME` 에서 돈다** — `cd` 를 안 붙였다가 `manage.py` 를 못 찾았고,
`env -i` 로 재 보고서야 알았다 (`devlog/20260829_002` 가 겪은 그 자리다). 시간별 트랙이 "한 시간 전으로 되돌리기"
