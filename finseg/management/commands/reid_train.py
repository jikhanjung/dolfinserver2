"""**이 길은 접었다 (2026-08-25).** 아래 설계는 *연속 프레임을 양성쌍으로* 쓰는
대조학습인데, 지금은 **사람이 붙인 개체 정답으로 닫힌 집합 분류**를 배운다
(`reid_cls`). 무엇이 갈랐는지는 재어 두었다.

    개체를 통째로 뺀 판 (열린 판)        좁은 후보    격자 전체
    ImageNet 기준선                      37.8%       21.2%
    + InfoNCE 대조학습                   31.7%        9.3%
    + batch-hard triplet                 35.9%       13.1%

    날로 가른 판 (닫힌 판 — **우리 자리**)
    기준선(kNN)                          25.5%       (top-5 57.7%)
    + 분류기                           **44.5%**     (top-5 68.6%)

**대조학습은 처음 보는 개체를 알아보라고 배우는 것**인데, 제주 남방큰돌고래는
개체군이 100~120이라 개체 수의 천장이 개체군 크기다. 처음 보는 개체를 만나는
일이 드물고, 묻는 것은 "카탈로그의 몇 번인가" 다 — 그러면 분류가 맞는 자다.
자세한 것은 `devlog/20260824_004` 와 `20260825_001`.

**그래도 남겨 둔다.** 아래 문서에 재어 둔 것들(연속 프레임 짝의 신뢰도, 그림
차이로 좁히면 10등 안이 37%→51%, `fliplr` 금지)이 다른 자리에서 쓰인다.
`--sigs` 캐시도 그 하나다.
