...
# ── 2차 목표 (2026-09-07) ────────────────────────────────────────────────────
#
# 1차(개체 60 · 조각 1,200)를 넘겼다 — 지금 개체 **67** · 조각 **1,704**.
# 다음 바퀴를 **개체 90 · 조각 4,000** 에서 돈다.
#
# **개체 수가 방아쇠이고 조각 수는 동행자다.** 조각 수를 방아쇠로 삼으면 안
# 되는 까닭이 실측으로 있다 — `--min-chips 20` 이 59.9 인데 **개체 수만 맞춘
# 대조군이 60.7 로 비겼다**(`devlog/20260831_002`). 개체당 조각을 더 줘도
# 모델은 안 는다.
#
# 실제로 그것이 다시 보였다: 개체판정 2,166 → 2,987 사이에 **조각은 +404 인데
# 개체는 +0** 이었다. `묶음 제안` 이 **모델이 확신하는 것**(=이미 아는 개체)을
# 내미는 화면이라 구조적으로 그 쪽으로 기운다.
#
# `individuals` = 90 — 개체군 100~120 의 **네 분의 셋**이다. 카탈로그가 덮는
# 정도가 열린 판(거절 문턱)에서 곧바로 걸린다.
#
# `chips` = 4,000 — 그 자체가 지렛대라서가 아니라, **개체를 찾으려면 격자를
# 훑어야 하고 훑으면 조각이 따라 는다.** 격자 7,912 의 절반이다.
#
# `per_individual` 은 **10 그대로 두고 쫓지 않는다** — 위 대조군에서 진 축이다.
# `days_per_individual` = 2 는 그대로 값이 있다(하루짜리 개체는 배우는 쪽이나
# 재는 쪽 한 곳에만 들어간다). 지금 52/67 이다.
GOAL = {
    "individuals": 90,
    "chips": 4000,
    "per_individual": 10,
