...
photo-ID 는 주로 뒷날의 notch으로 개체를 가리므로, 뒷날만 보이는 지느러미는
...
# 기준으로 믿어도 되나" 다.
#
# **어느 쪽인지를 남기는 이유**: re-ID 는 주로 뒷날 notch으로 개체를 가린다.
# 뒷삽입점이 짐작이면 notch 를 재는 쪽 끝이 흔들려 값이 크게 떨어지지만,
...
class Individual(models.Model):
    ...
    **닮음을 잴 때는 절대 섞지 않는다** (`reid.frame` 의 부호 주석).
    ...
    # `unid`   **이 사진으로는 식별이 안 되는 것.** 지느러미는 맞는데 사람이
    #          개체를 알아볼 수 없다 — `hold`(알아볼 수 있는데 자리를 모름)와
    #          다르다. 학습·성적에는 안 들어가고, 거절 문턱의 **음성 예제**로
    #          쓴다 (`TODOs`)
    #
    # **`unid` 를 둘로 가른다** (2026-09-07). 가르는 축은 하나다 —
    # **다른 사진이면 되나.**
    #
    # `unid_photo_qual`   **사진이 나쁘다** — 흔들려 흐리거나 파도·물방울에
    #                    가렸다. 같은 개체를 다른 날 잘 찍으면 알아볼 수 있다
    # `unid_too_smooth`  **뒷날이 밋밋하다** — 그림은 또렷한데 notch 가 없다.
    #                    **다른 사진이어도 안 된다.** 이 방법의 천장을 가리킨다
    #
    # **이름이 곧 뜻이라야 한다** — `unid_img`·`unid_fin` 처럼 짧게 짧게 두었다가
    # 고쳤다. 주석을 봐야 뜻이 서는 이름은 세는 자리마다 다시 물어야 한다
    #
    # **둘을 자동으로 가를 수 있는지는 아직 모른다.** 2026-09-07 에 자를 넷
    # 재고 다 어긋났다(라플라시안 · 윤곽 기울기 · 경계 너비 · focus stacking 식
    # 상대 분위). **넷 모두 `unid` 안에서 확신 구간별로 평평했다** — 그러면
    # 흐림이 별로 안 갈리거나 확신이 흐림을 안 따라가거나인데, 어느 쪽이든
    # **"확신이 낮다" 를 "흐리다" 대신 쓸 수 없다.**
    # 갈래를 나눠 두는 것은 **사람이 갈라 준 정답을 담을 자리**를 만드는 일이고,
    # 자는 그 정답이 몇십 장 모인 뒤에 고른다 (`TODOs`).
    #
    # **옛 `unid` 는 그대로 둔다** — "아직 안 가른 것" 이다. 390장을 다시 보게
    # 하지 않고, 앞으로 누를 때부터 갈리면 된다.
    KINDS = [("", "개체"), ("hold", "임시보관함"), ("notfin", "지느러미 아님"),
             ("unid", "식별 불가능"),
             ("unid_photo_qual", "식별 불가능 · 사진 품질"),
             ("unid_too_smooth", "식별 불가능 · 뒷날이 밋밋")]

    #: `unid` 갈래 전부. 세거나 거를 때 **이 하나를 본다** — 갈래가 늘 때마다
    #: 여기저기 문자열을 고치면 한 곳이 빠지고, 그때 그 조각들이 조용히
    #: 학습이나 성적에 든다
    UNID_KINDS = ("unid", "unid_photo_qual", "unid_too_smooth")
    # **20 이다.** `unid_too_smooth` 가 15자다 — 이름이 뜻을 담게 두려면
    # 칸이 그만큼 있어야 한다
