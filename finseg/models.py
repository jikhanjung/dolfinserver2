...
class Individual(models.Model):
    ...
    **닮음을 잴 때는 절대 섞지 않는다** (`reid.frame` 의 부호 주석).
    ...
    #
    # 실측이 그 둘을 **정반대로** 가른다 (2026-09-07 · 배포된 3자 앙상블):
    # 흐린 것은 확신이 낮아 지금 자로 잘 걸리는데(`unid` 전체와 개체 조각의
    # AUROC 0.971), **밋밋한 것은 확신이 되레 높다** — 매끈한 뒷날이 카탈로그의
    # 다른 매끈한 개체와 잘 닮아 **자신 있게 잘못 찍는다.** 확신 하나로는
    # `unid_fin` 을 절대 못 거른다.
    #
