...
review/            격자 검토 앱 — 기본값으로 두고 예외만 누른다
                   ...
                   **저쪽에서 아니라 한 것**(GCP 가 `지느러미 아님` 에 넣은 것 —
                   진짜 분류는 여기서 사람이 골라 적는다).
...
| `HANDOFF.md` | **지금** 상태 |
| `TODOs.md` | **앞으로** 할 일 |
| `devlog/` | **지나간** 것의 영구 기록 (`README.md` 가 색인) |
| `docs/지금의_구조.md` | **한 장으로 본 구조** — 자리 셋 · 자료 한 줄기 · 규칙 한 곳씩 |
| `docs/여기까지_온_길.md` | **무엇을 해 보고 무엇을 버렸나** — 버린 것과 버리게 만든 자 |
| `docs/re-ID_1차_바퀴_기록.md` | **무엇으로 배웠고 무엇이 나왔나** — 개체 67 · 조각 1,300 · 3자 앙상블 89.9. 2차 바퀴 뒤에 나란히 놓고 읽는다 |
| `docs/` 나머지 | 물음에 답한 것 (닫힌 판과 열린 판 · 이미지 분류의 계보) |

**처음 오는 사람은 `docs/` 둘을 먼저 읽는 편이 빠르다** — `HANDOFF` 는 지금
자리를 적느라 길고, `devlog/` 는 날짜순이라 줄기가 안 보인다.
...
- **뒷날의 흠은 `notch`.** 우리말 `결각` 을 쓰던 것을 걷었다 — 이 과제의 축을
  가리키는 말이라 자주 나오는데, 읽는 사람이 한 번 더 새겨야 하는 말이었다.
  코드의 `rough`·`roughness` 는 그대로 둔다 (격자 `items.json` 이 그 이름을
  들고 있어, 바꾸면 격자를 다시 만들 때까지 두 이름이 함께 돌아다닌다).
  **`devlog/` 는 안 고친다** — 지나간 기록이다
- **되풀이되는 경향은 `무늬` 말고 `패턴`.** `무늬` 는 이 뜻으로는 어색하다 —
  "같은 패턴이 B 에서도 나온다" 가 자연스럽다 (2026-09-01 지적)
